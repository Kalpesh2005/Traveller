...
// -------------------- CORS Setup --------------------
// Allowlist of origins that can access this API. Add other dev ports (e.g. 3001)
// or production frontends here. You can also set an env var `ALLOWED_ORIGINS`
// as a comma-separated list to avoid editing this file for quick changes.
const defaultAllowed = [
  'http://localhost:3000',
  'http://localhost:3001', // added so local frontend running on 3001 is allowed
...
const corsOptions = {
  origin: function (origin, callback) {
    if (!origin || allowedOrigins.includes(origin)) {
      ...
      // Pass an error with status so callers (and the server) can respond cleanly
...
// Simple error handler to return CORS errors with a status code instead of crashing
